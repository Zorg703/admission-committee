...
public class ShowRegisterOnSpecialityUsers implements Command {
    ...
    private static final String REGISTER_USER_PAGES="register_user_pages";
    ...
    public Router execute(SessionRequestContent content) {
        ...
        try {
            ...
            if(optionalSpeciality.isPresent() && optionalUsers.isPresent()){
                ...
                if(users.size()>10){
                    int pages=(int) Math.ceil((double)users.size()/10);
                    users=users.subList(0,10);
                    content.setSessionAttribute(REGISTER_USER_PAGES,pages);
                }
