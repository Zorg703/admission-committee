package by.mordas.project.command;

/***
 Author: Sergei Mordas
 Date: 18.04.2018
 ***/
public class PageConstant {
    public static final String PAGE_LOGIN="views/jsp/common/log_in.jsp";
    public static final String PAGE_SHOW_USERS="views/jsp/admin/find_all_users.jsp";
    public static final String PAGE_MAIN="views/jsp/common/main.jsp";
    public static final String PAGE_SHOW_ALL_FACULTY ="views/jsp/user/show_all_faculty.jsp";
    public static final String PAGE_SHOW_SPECIALITIES="views/jsp/user/show_specialities.jsp";
    public static final String PAGE_REGISTRATION="views/jsp/common/sign_up.jsp";
    public static final String PAGE_USER_SUBJECTS="views/jsp/user/user_subjects.jsp";
    public static final String PAGE_USER_SUCCESS="views/jsp/user/user_success.jsp";
    public static final String PAGE_SUCCESS_REGISTRATION="views/jsp/common/success_registration.jsp";
    public static final String PAGE_CHANGE_PASSWORD="views/jsp/user/change_user_password.jsp";
    public static final String PAGE_REGISTER_ON_FACULTY="views/jsp/user/register_on_faculty.jsp";
    public static final String PAGE_ADD_FACULTY="views/jsp/admin/add_faculty.jsp";
    public static final String PAGE_DELETE_FACULTY="views/jsp/admin/delete_faculty.jsp";
    public static final String PAGE_ADMIN_SUCCESSFUL="views/jsp/admin/admin_successful.jsp";
    public static final String PAGE_ADD_SPECIALITY="views/jsp/admin/add_speciality_on_faculty.jsp";
    public static final String PAGE_FIND_USER_BY_ID="views/jsp/admin/find_user_by_id.jsp";
    public static final String PAGE_DELETE_SPECIALITY="views/jsp/admin/delete_speciality.jsp";
    public static final String PAGE_ERROR="/views/jsp/error/error.jsp";
    public static final String PAGE_FIND_ALL_FACULTY="views/jsp/admin/find_all_faculty.jsp";
    public static final String PAGE_SHOW_ALL_SPECIALITIES="views/jsp/admin/show_all_specialities.jsp";
    public static final String PAGE_FIND_ALL_USERS_REGISTER_ON_SPECIALITY="views/jsp/admin/find_all_users_register_on_speciality.jsp";
    public static final String PAGE_SHOW_USER_STATUS="views/jsp/user/show_user_status.jsp";
    public static final String PAGE_UPDATE_FACULTY="views/jsp/admin/update_faculty.jsp";
    public static final String PAGE_UPDATE_SPECIALITY="views/jsp/admin/update_speciality.jsp";
    public static final String PAGE_SHOW_ACCEPTED_USERS="views/jsp/admin/show_accepted_users_on_speciality.jsp";
    public static final String PAGE_USER_DATA="views/jsp/user/user_data.jsp";
    public static final String PAGE_CHOOSE_FACULTY_AND_SPECIALITY="views/jsp/admin/choose_faculty_and_speciality.jsp";
    public static final String PAGE_CANCEL_REGISTRATION="views/jsp/user/cancel_registration.jsp";
    public static final String PAGE_CHANGE_REGISTRATION_DATE="views/jsp/admin/change_speciality_register_date.jsp";
    public static final String PAGE_CHOOSE_FACULTY_TO_RESULT="views/jsp/admin/choose_faculty_to_result.jsp";
    public static final String PAGE_SHOW_STATE_OF_ADMISSION_COMMITTEE="views/jsp/admin/show_state_of_admission_committee.jsp";
}


